...
function isCreditCardNumber(ccn) {
  return String(ccn)
    ...
    .reduce((accumulator, el) => accumulator + el) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let number = num;
  while (number > 9) {
    number = Number(number.toString().split('').reduce((a, b) => Number(a) + Number(b)));
  }
  return number;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (index % 2 === 0) {
      arr.push(str[i]);
    } else if (arr[arr.length - 1] === brackets[index - 1]) {
      arr.pop();
    } else return false;
  }
  return !arr.length;
...
function getCommonDirectoryPath(pathes) {
  ...
  array.map((el, index) => {
    if (el.length < minArray.length) {
      minArray.length = el.length;
  ...
  for (let i = 0; i < minArray.length; i += 1) {
    const everyoneIsEqual = array.every((el) => {
      if (el[i] === array[minArray.index][i]) {
        return true;
      }
      return false;
    });
  ...
  let result = array[minArray.index];
...
function getMatrixProduct(m1, m2) {
  const arr = [];
  function calc(matrix1, matrix2, j) {
    let res = 0;
    matrix1.forEach((value, index) => {
      res += value * matrix2[index][j];
    });
    return res;
  }
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[0][0] === position[1][1] && position[0][0] === position[2][2]) {
    if (position[1][1] !== undefined) return position[1][1];
  }
